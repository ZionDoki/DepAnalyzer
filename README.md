...
- **主要组件**:
  - `dependency_manager.py` - 依赖管理器与组合式 Fetch 入口
  - `dependencies/fetchers/` - 插件式源码获取器（Fetcher）实现与注册表
    - `hvigor_fetcher.py`：基于 REST 元信息 + Git 的 Hvigor 源码获取，实现按发布时间选择最近提交；若无 `repository` 则生成 license-only 节点，不做文件级解析
