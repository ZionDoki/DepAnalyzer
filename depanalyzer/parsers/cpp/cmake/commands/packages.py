...
class PackagesCommandHandler(CommandHandler):
    ...
    def handle(
        ...
        """Handle package management commands by publishing CMAKE_EXTERNAL_PACKAGE_REFERENCED events.
    ...
    def _handle_find_package(
        ...
        event = Event(
            event_type=EventType.CMAKE_EXTERNAL_PACKAGE_REFERENCED,
        ...
        log.debug("Published CMAKE_EXTERNAL_PACKAGE_REFERENCED for find_package: %s", pkg)
    ...
    def _handle_fetchcontent_makeavailable(
        ...
        for arg in args:
            ...
            if proj_info:
                ...
                self.eventbus.publish(event)
                log.info("Published CMAKE_EXTERNAL_PACKAGE_REFERENCED for FetchContent: %s (repo: %s, tag: %s)",
                ...
                self.eventbus.publish(event)
                log.warning(
                    "Published CMAKE_EXTERNAL_PACKAGE_REFERENCED without Declare info: %s", project_name
                )
